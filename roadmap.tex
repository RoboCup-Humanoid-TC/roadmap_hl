...
\begin{center}
  ...
  \vspace{\stretch{4}}
  {\bfseries \added{XXX 2020} \removed{June 13th 2019}}
\end{center}
...
\subsection{RoboCup 2021 changes}
\added{
  According to the votes of the league, this year should be a year with minor
  changes or simple fixes to flaws in the rules.

  The following changes are already planned according to the vote from the
  teams:
  \begin{itemize}
  \item WiFi restriction changes: with the increasing collaboration between
    robots and the complexity of the rules, robots are relying more on WiFi.
    Therefore, a device need to be developed to ensure two key-points:
    \begin{enumerate}
    \item WiFi quality: a minimal bandwidth should be ensured at any location
      in the field.
    \item Respect of the rules: the bandwidth allowed for each robot should be
      limited to ensure a fair competition and the robots not in play should not
      be able to send messages.
    \end{enumerate}
  \end{itemize}

  Other changes to be discussed/voted
}

\subsection{RoboCup 2023 changes}
